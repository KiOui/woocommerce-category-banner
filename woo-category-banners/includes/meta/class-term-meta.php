			...
			wp_nonce_field( basename( __FILE__ ), $this->get_nonce_name() ); ?>
			 ...
			 value="<?php echo esc_attr( $option['value'] ); ?>">
									...
									case 'image':
										...
										?>
										<div id="<?php echo esc_html( 'wcb_' . $field['id'] ); ?>" style="float: left; margin-right: 10px;"><img style="display: 
															<?php
															if ( null === $image_url ) {
																echo 'none';
															} else {
										...
										<div style="line-height: 60px;">
											...
											<button type="button" class="<?php echo esc_attr( 'wcb_upload_image_button_' . $field['id'] ); ?> button"><?php esc_html_e( 'Upload/Add image', 'woo-category-banners' ); ?></button>
											<button type="button" class="<?php echo esc_attr( 'wcb_remove_image_button_' . $field['id'] ); ?> button"><?php esc_html_e( 'Remove image', 'woo-category-banners' ); ?></button>
										</div>
										<script type="text/javascript">

											// Only show the "remove image" button when needed
											if ( ! jQuery( '#<?php echo esc_html( $field['id'] ); ?>' ).val() ) {
												jQuery( '.<?php echo esc_attr( 'wcb_remove_image_button_' . $field['id'] ); ?>' ).hide();
											}

											// Uploading files
											var file_frame;

											jQuery( document ).on( 'click', '.<?php echo esc_attr( 'wcb_upload_image_button_' . $field['id'] ); ?>', function( event ) {
												...
												file_frame = wp.media.frames.downloadable_file = wp.media({
													title: '<?php esc_html_e( 'Choose an image', 'woo-category-banners' ); ?>',
													button: {
														text: '<?php esc_html_e( 'Use image', 'woo-category-banners' ); ?>'
													},
													multiple: false
												});
											...
											jQuery( document ).on( 'click', '.<?php echo esc_attr( 'wcb_remove_image_button_' . $field['id'] ); ?>', function() {
